...
class Engine(AbstractEngine):
    ...
    def run(self):
        ...
        Raises:
            CPUEngineException: If engine fails to run.
        """
        if not self._inference_engine or not self._input_module:
            raise CPUEngineException(
        ...
        try:
            ...
            raise CPUEngineException(exc) from exc
        finally:
            self._cleanup()
